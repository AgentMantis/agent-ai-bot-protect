...
(async function build() {
  const distPath = path.join(__dirname, 'dist/bot-shield/browser');
  const wpPluginPath = path.join('D:', 'source', 'repos', 'BotShield', 'plugins', 'bot-shield', 'dist');
  ...
  try {
    ...
    const bundle = await rollup.rollup({
      input: entryFile,
      output: {
        file: path.join(wpPluginPath, 'bot-shield.js'),
        format: 'es',
    ...
    if (styleFile) {
      await fs.copy(
        ...
        path.join(wpPluginPath, 'bot-shield.css')
      );
      console.log('Styles copied successfully!');
    }

    // Read and update the JS file
    const jsFilePath = path.join(wpPluginPath, 'bot-shield.js');
    ...
    const cssFilePath = path.join(wpPluginPath, 'bot-shield.css');
    let cssContent = await fs.readFile(cssFilePath, 'utf8');
    cssContent = cssContent.replace(
      /url\(['"]?(\.?\/?)?assets\//g,
      'url(/wp-content/plugins/bot-shield/dist/assets/'
    );
